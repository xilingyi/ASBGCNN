...
def getlayers(crystal_del, layer_thresthold):
    symbol = crystal_del.symbol_set 
    element_list_noNum_old = list(map(lambda x: x.strip(string.digits), crystal_del.formula.split(' ')))
    idx = [element_list_noNum_old.index(symbol[i]) for i in range(len(symbol))]
    element_list_old = crystal_del.formula.split(' ')
    element_list = [element_list_old[i] for i in idx]
#     element_list.sort(key=lambda l: int(re.findall('\d+', l)[0]))
    ...
    element_list_noDig = list(map(lambda x: x.strip(string.ascii_letters), element_list))
    element_list_noDig_int = list(map(int, element_list_noDig))
    element_list_noDig_id = sorted(range(len(element_list_noDig_int)), key=lambda k: element_list_noDig_int[k], reverse=False)
    LAYER_DISTRIB_SORT = sorted(LAYER_DISTRIB, key=lambda x: element_list_noDig_id[LAYER_DISTRIB.index(x)], reverse=False)
    countlayers_list = {}
    ...
    for i in range(len(countlayers_list)):
        ...
        layer_num = int(LAYER_DISTRIB_SORT[i])
        x_cartesian = []
        y_cartesian = []
        z_cartesian = []
        for j in list_: 
    ...
    #基于层高重新排序layerscount_total
    layer_Zmean = []
    for i in range(len(list(layerscount_total.values()))):
        Z_ = []
        for j in list(layerscount_total.values())[i]:
            Z_.append(crystal_del.frac_coords[j][2])
        Z_mean = np.mean(Z_)
        layer_Zmean.append(Z_mean)
    for i in range(len(layer_Zmean)):
        layerscount_total[layer_Zmean[i]] = layerscount_total.pop(i+1)  
    layerscount_total_order=dict(sorted(layerscount_total.items(),key=lambda x:x[0],reverse=False))
    count = 1
    for i in list(layerscount_total_order.keys()):
        layerscount_total_order[count] = layerscount_total_order.pop(i)
        count = count + 1
    return layerscount_total_order
...
def insert_singleAtom(crystal, surface_layerIndex_p, singleAtom_Index, fraction_coords_p, fraction_coords):
    ...
    surface_layerIndex[z_index] = surface_layerIndex[z_index] + [max_num]
...
def find_neighbor(lattices, expanded_lattices, elements, cut_rds, nbr_type):
    ...
    for i in range(lattice_num):
        ...
        for j in range(expanded_lattice_num):
            ...
            cond1 = np.linalg.norm(lattices[:,i] - expanded_lattices[:,j]) < cut_rds
...
#             print(np.linalg.norm(lattices[:,i] - expanded_lattices[:,j]))
#             print((atom_features[elements[i]]['radius'] + atom_features[elements[j%len(elements)]]['radius'] + tolerance))
            if nbr_type == 'T':
                if cond1 and cond2:
...
def bond_construction(elements,connectivity,distances, features, cut_rds, CATEGORY_NUM, step=0.2):
#     BOND_CATEGORY_NUM = 1
    BOND_CATEGORY_NUM = len(np.arange(0, cut_rds+step, step))
    bond_vectors = []
    neighbor_indices = []
    ...
    for i in range(len(elements)):
        ...
        for j in range(neighbor_num[i]):
            ...
            bond_vector[j] = bond_encoding(distances[count], cut_rds, step)
            count += 1
...
def poscar_to_graph(name, poscar_dir, formula, parameters, cut_rds, nbr_type):
    ...
    _, _, dot_layer_index, total_layer_index, crystal, _ = countlayers(name, crystal_ori, layer_thresthold, num_surface_layer, num_inter_layer)
    dot_layer_index2 = sum(sorted(dot_layer_index.values()),[])
    total_layer_index2 = sorted(sum(total_layer_index.values(),[]))
#     inter_layer_index2 = np.array([value for value in sorted(inter_layer_index.values())]).reshape(-1)
#     print(crystal.species[i])
    element_total = [str(crystal.species[i]) for i in total_layer_index2]
    element_dot = [str(crystal.species[-1])]
    ...
    dot_atom_num = [int(i) for i in Counter(element_dot).values()]
    dot_total_atom_num = sum(dot_atom_num)
    total_atom_num = [int(i) for i in Counter(element_total).values()]
    total_total_atom_num = sum(total_atom_num) 

    total_atoms = crystal.frac_coords[total_layer_index2]

    lattices = np.transpose(np.matmul(total_atoms, trans_matrix))
    expanded_lattices = unit_cell_expansion_slab(total_atoms, trans_matrix)
    connectivity, distances = find_neighbor(lattices, expanded_lattices, element_total, cut_rds, nbr_type)

    atom_vectors = np.zeros([total_total_atom_num, CATEGORY_NUM], dtype=np.float32)
    bond_num = np.zeros([total_total_atom_num],dtype=np.int32)
    for i in range(total_total_atom_num):
        atom_vectors[i] = atom_encoding(element_total[i])
    bond_vectors, neighbor_indices = bond_construction(element_total, connectivity, distances, features, cut_rds, CATEGORY_NUM)
    for i in range(len(bond_vectors)):
        bond_num[i] = bond_vectors[i].shape[0]

    atom_vectors_single = atom_vectors[-1]
    bond_vectors = [bond_vectors[-1]]
    neighbor_indices = [neighbor_indices[-1]]

    atom_vectors_nbr = []
    for i in range((neighbor_indices[0]).shape[0]):
        idx = int(neighbor_indices[0][i][1])
        atom_vectors_nbr.append(atom_vectors[idx])
        neighbor_indices[0][i][1] = i
    atom_vectors_single = np.array(atom_vectors_single)
    atom_vectors_nbr = np.array(atom_vectors_nbr)
#     print(atom_vectors_single, '**', atom_vector_nbr, '**', bond_vectors,  '**', neighbor_indices,  '**', name)
    return atom_vectors_single, atom_vectors_nbr, bond_vectors, neighbor_indices