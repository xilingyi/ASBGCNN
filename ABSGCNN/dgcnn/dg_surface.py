...
def poscar_to_graph(name, poscar_dir, formula, parameters, cut_rds, nbr_type):
    ...
    surface_layer_index, _, _, _, _, crystal = countlayers(name, crystal_ori, layer_thresthold, num_surface_layer, num_inter_layer)
    surface_layer_index2 = sum(sorted(surface_layer_index.values()),[])
#     surface_layer_index2 = np.array([value for value in sorted(surface_layer_index.values())]).reshape(-1)
#     print(crystal.species[i])
    element_surface = [str(crystal.species[i]) for i in surface_layer_index2]
    ...
    surface_atom_num = [int(i) for i in Counter(element_surface).values()]
    surface_total_atom_num = sum(surface_atom_num)
    
    surface_atoms = crystal.frac_coords[surface_layer_index2]
    lattices = np.transpose(np.matmul(surface_atoms, trans_matrix))
    expanded_lattices = unit_cell_expansion_slab(surface_atoms, trans_matrix)
    connectivity, distances = find_neighbor(lattices, expanded_lattices, element_surface, cut_rds, nbr_type)
    
    atom_vectors = np.zeros([surface_total_atom_num, CATEGORY_NUM], dtype=np.float32)
    bond_num = np.zeros([surface_total_atom_num],dtype=np.int32)
    for i in range(surface_total_atom_num):
        atom_vectors[i] = atom_encoding(element_surface[i])
    bond_vectors, neighbor_indices = bond_construction(element_surface, connectivity, distances, features, cut_rds, CATEGORY_NUM)
