...
def countlayers(name, crystal, layer_thresthold, surface_layerNum, inter_layerNum):
    ...
    if name.split('_')[-1][0].upper() == 'S':
        surface_layerIndex_del = dict_slice(layerscount, layer_num - surface_layerNum, layer_num)
        ...
        surface_layerIndex = dict_slice(layerscount, layer_num - surface_layerNum, layer_num)
        crystal_new = crystal.copy()
    inter_layerIndex = dict_slice(layerscount, 0, inter_layerNum)
    return surface_layerIndex, inter_layerIndex, crystal_new
...
def poscar_to_graph(name, poscar_dir, formula, parameters, cut_rds, nbr_type):
    ...
    surface_layer_index, _, crystal = countlayers(name, crystal_ori, layer_thresthold, num_surface_layer, num_inter_layer)
#     surface_layer_index2 = np.array([value for value in sorted(surface_layer_index.values())]).reshape(-1)
    ...
    elements = [str(crystal.species[i]) for i in surface_layer_index2]
    ...
    atom_num = [int(i) for i in Counter(elements).values()]
    total_atom_num = sum(atom_num)
    
    atoms = crystal.frac_coords[surface_layer_index2]

    lattices = np.transpose(np.matmul(atoms, trans_matrix))
    expanded_lattices = unit_cell_expansion_slab(atoms, trans_matrix)

    connectivity, distances = find_neighbor(lattices, expanded_lattices, elements, cut_rds, nbr_type)
