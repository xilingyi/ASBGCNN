...
def get_weight_parameters(dataset):
    targets = []
    num = len(dataset)
    for i, (_, _, target, _) in enumerate(dataset): 
...
def SIget_train_val_test_loader(dataset, collate_fn=default_collate,
...
def SIcollate_pool(dataset_list):
    """
    ...
    batch_cif_ids: list
    """
    batch_atom_fea_i, batch_nbr_fea_i, batch_nbr_fea_idx_i, batch_atom_fea_s, batch_nbr_fea_s, batch_nbr_fea_idx_s = [], [], [], [], [], []
    crystal_atom_idx_i, crystal_atom_idx_s, batch_target = [], [], []
    batch_cif_ids = []
    base_idx_i = 0
    base_idx_s = 0
    for i, ((atom_fea_i, nbr_fea_i, nbr_fea_idx_i), (atom_fea_s, nbr_fea_s, nbr_fea_idx_s), target, cif_id)\
        ...
        batch_nbr_fea_idx_i.append(torch.where(nbr_fea_idx_i != -1, base_idx_i + nbr_fea_idx_i, nbr_fea_idx_i))# 不等于-1加base_idx_i           
    ...
    return (torch.cat(batch_atom_fea_i, dim=0),
...
class SIStructureData(Dataset): 
    """
        
        
    """
    def __init__(self, root_dir, structures_formula, layersNum_parameter, cut_rds= [8,8], nbr_type=['T', 'T'], random_seed=123):
        ...
        training_inter_num, self.training_atom_i, self.training_bond_i, self.training_bond_index1_i, self.training_bond_index2_i, training_atom_num_i, training_bond_num_i, training_surface_num, self.training_atom_s, self.training_bond_s, self.training_bond_index1_s, self.training_bond_index2_s, training_atom_num_s, training_bond_num_s = data_preparation(sample_id, self.root_dir,                                                                                  self.structures_formula, self.parameters, self.cut_rds, self.nbr_type)
    ...
    def __getitem__(self, idx):
        ...
        return (atom_fea_i, nbr_fea_i, nbr_fea_index_i), (atom_fea_s, nbr_fea_s, nbr_fea_index_s), target, cif_id        
