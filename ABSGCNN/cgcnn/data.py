...
from cgcnn.cg_encoding import *

def get_weight_parameters(dataset):
    targets = []
    num = len(dataset)
    for i, (_, target, _) in enumerate(dataset): 
...
class AtomCustomJSONInitializer(AtomInitializer):
    ...
    """
    def __init__(self):
        elem_embedding = get_atom_onehots()
...
class StructureData(Dataset): 
    ...
    cif_id: str or int
    """
    def __init__(self, root_dir, structures_formula, max_num_nbr=12, cut_rds=8, dmin=0, step=0.2,
                 random_seed=123):
        self.root_dir = root_dir
        self.max_num_nbr, self.cut_rds = max_num_nbr, cut_rds
        ...
        self.gdf = GaussianDistance(dmin=dmin, dmax=self.cut_rds, step=step)
    ...
    def __getitem__(self, idx):
        ...
        if self.structures_formula == 'cif':
            crystal = Structure.from_file(os.path.join(self.root_dir,
                                                   cif_id+'.cif'))#获取cif结构
        elif self.structures_formula == 'vasp':
            crystal = Structure.from_file(os.path.join(self.root_dir,
                                                   cif_id+'.vasp'))#获取vasp结构            
        atom_fea = np.vstack([self.ari.get_atom_fea(crystal[i].specie.number)
                              for i in range(len(crystal))])
        atom_fea = torch.Tensor(atom_fea)
        all_nbrs = crystal.get_all_neighbors(self.cut_rds, include_index=True)
        all_nbrs = [sorted(nbrs, key=lambda x: x[1]) for nbrs in all_nbrs]#按元组第2个元素(distance)排序  
        nbr_fea_idx, nbr_fea = [], []
        for nbr in all_nbrs:
            if len(nbr) < self.max_num_nbr:
                ...
                nbr_fea.append(list(map(lambda x: x[1], nbr)) +
                               [self.cut_rds + 1.] * (self.max_num_nbr -
                                                     len(nbr)))
            else:
